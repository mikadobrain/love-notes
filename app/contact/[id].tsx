...
export default function ContactDetailScreen() {
  ...
  function handleChipPress(chipKey: string) {
    const chipText = t(chipKey).replace(/\.{3}$/, '') + ' ';
            ...
            >
              {CHIP_KEYS.map((key) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    newMessage === t(key).replace(/\.{3}$/, '') + ' ' && styles.chipActive,
                  ]}
                  onPress={() => handleChipPress(key)}
                  activeOpacity={0.7}
                >
                  <Text style={[styles.chipText, newMessage === t(key).replace(/\.{3}$/, '') + ' ' && styles.chipTextActive]}>
                    {t(key)}
                  </Text>
                </TouchableOpacity>
              ))}
